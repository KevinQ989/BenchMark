import { SafeAreaView, View, Text, FlatList, StyleSheet, TouchableOpacity, Button } from "react-native";
...
const AddExercisesScreen = () => {
    ...
    const toggle = (item: ExerciseInfo) => {
        setSelected(prev => {
            const isSelected = prev.some(exercise => exercise.exerciseName === item.exerciseName);
            if (isSelected) {
                return prev.filter((exercise => exercise.exerciseName !== item.exerciseName));
            } else {
                return [...prev, item];
            }
        })
    ...
    return (
        <>
            <Stack.Screen options={{
                    headerTitle: "",
                    headerBackTitle: "",
                    headerRight: () => <Button onPress={saveSelected} title="Add Exercises" />
                }}
            />
